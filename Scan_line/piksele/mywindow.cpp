    ...
    *mvImg = *img;
    if(startX != 0 && startY != 0)
    {
        ...
        switch(spr)
        {
            ...
            break;
            case 3:
                floodFill(konX, konY, img->pixel(konX,konY));
            break;
            case 4:
                obliczenia2(startX, startY, konX, konY);
            break;
            case 5:
                rysujwielokat();
            break;
    ...
    void MyWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    void MyWindow::rysujpiksel(int x, int y, QImage *obrazek)
    {
            if(!(x < 0 || y < 0 || x >= szer || y >= wys))
            {
                ...
                ptr[szer*4*y+4 + 4*x] = kolorek.blue();
                ptr[szer*4*y+4 + 4*x + 1] = kolorek.green();
                ptr[szer*4*y+4 + 4*x + 2] = kolorek.red();

                ptr[szer*4*y-4 + 4*x] = kolorek.blue();
                ptr[szer*4*y-4 + 4*x + 1] = kolorek.green();
                ptr[szer*4*y-4 + 4*x + 2] = kolorek.red();

                ptr[szer*4*y + 4*x+4] = kolorek.blue();
                ptr[szer*4*y + 4*x+4 + 1] = kolorek.green();
                ptr[szer*4*y + 4*x+4 + 2] = kolorek.red();
...
void MyWindow::floodFill(int x1, int y1, QColor zapodany)
{
    std::stack <punkt> punkty;
    punkt pierw;
    pierw.x = x1;
    pierw.y = y1;
    punkty.push(pierw);
...
void MyWindow::Scan_Line()
{
    int x, y;
    int licznik = 0;
    bool znal_pierw = false;
    bool znal_drugi = false;
    punkt pocz, kon;
    pocz.x = -1;
    pocz.y = -1;

    QColor tlo = img->pixel(1,1);
    QColor pom, pom2, pom3;
    QColor somsiad;
    ...
    /*for(y = 1; y < wys; y++)
    {
        for(x = 1; x < szer; x++)
        {
            ...
            if(pom != tlo)
            {
                pom2 = img->pixel(x+2, y);
                pom3 = img->pixel(x+1,y);

                if(pom != pom3 && pom3 == pom2)
                {
                    licznik++;
                    //if(pom != )
                }
    ...
    for(y = 1; y < wys; y++)
    {
        ...
        {
            pom = img->pixel(x,y);
            if(pom != img->pixel(x-1,y))
            {
                if(pom != tlo && znal_pierw == false)
                {
                    pocz.x = x;
                    pocz.y = y;
                    znal_pierw = true;
                ...
                else if(pom != tlo && znal_pierw == true)
                {
                    kon.x = x;
                    kon.y = y;
                    znal_drugi = true;
                ...
                if(znal_drugi == true)
                {
                    obliczenia2(pocz.x, pocz.y, kon.x, kon.y);
                    znal_pierw = false;
                    znal_drugi = false;
                }
            }
